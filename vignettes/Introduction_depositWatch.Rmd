...
3. summary(): This method provides a concise view of the anomaly detection results stored in a deposit_dataset object after running detect_anomalies(). It prints two summaries to the console: a monthly summary showing the number of anomalies per customer and a daily-level table of flagged transactions. The monthly summary can optionally be exported to an Excel file or a csv file.If excel path is provided, only monthly summary is exported and if the csv path is provided, both the datasets are exported.   
By separating computation (detect_anomalies()) from presentation (summary()), the design ensures that users can quickly access the results, inspect anomaly patterns, and optionally generate external reports, all without recalculating the rolling statistics or flags.
...
ds1 <- summary(ds,csv_path = "D:/Results")
```
...
```{r pressure, fig.width = 7, fig.height = 5, echo=FALSE}
plot(ds)
```
...
This summary table helps users quickly identify customers with irregular behaviour and understand the type of anomalies detected.  
  
For more detailed analysis, users can explore the daily-level data stored in obj$flagged_data.  
The example below filters out any unflagged rows to focus on valid flagged transactions:  

```{r analysis, eval=FALSE}
valid_data <- dplyr::filter(obj$flagged_data, !is.na(flag))
```
